...
1. **Download models and auto-configure (recommended):**
   ```bash
   # This automatically downloads models and creates the config file
   python scripts/download_models_hf.py
   ```

2. **Or manually copy template and configure:**
   ```bash
   cp configs/magic-pdf-gpu.template.json ~/magic-pdf.json
   # Then update paths in the configuration file
   ```
